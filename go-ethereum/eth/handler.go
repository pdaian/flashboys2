...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	defer msg.Discard()
	// ssss := fmt.Sprintf("flashboys2 msg hit: %X", msg.Code)
	// log.Debug(ssss)
		...
		for _, block := range announces {
			if !pm.blockchain.HasBlock(block.Hash, block.Number) {
				...
				if !pm.downloader.Synchronising() {
					...
					log.Info("flashboys2 saw new block hash message but still synchronising")
				}
			}
		}
	...
	case msg.Code == TxMsg:
		...
		for i, tx := range txs {
			...
			if !pm.downloader.Synchronising() {
				...
				log.Info("flashboys2 saw new transaction but still synchronising")
			}
